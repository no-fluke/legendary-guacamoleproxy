...
from vars import API_ID, API_HASH, BOT_TOKEN, OWNER, CREDIT, AUTH_USERS, TOTAL_USERS, PROXY

# Proxy helper — adds --proxy flag to yt-dlp shell commands if PROXY is set
def proxy_flag():
    ...
    try:
        for i in range(arg-1, len(links)):  # Iterate over each link
            ...
            if "youtube.com" in url or "youtu.be" in url:
                ...
                cmd = f'yt-dlp {proxy_flag()} -x --audio-format mp3 --cookies {cookies_file_path} "{url}" -o "{name}.mp3"'
...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
                cmd = f'yt-dlp {proxy_flag()} -o "{name}.%(ext)s" -f "bestvideo[height<={raw_text2}]+bestaudio" --hls-prefer-ffmpeg --no-keep-video --remux-video mkv --no-warning "{url}"'
            ...
            if "jw-prod" in url:
                cmd = f'yt-dlp {proxy_flag()} -o "{name}.mp4" "{url}"'
            elif "webvideos.classplusapp." in url:
               cmd = f'yt-dlp {proxy_flag()} --add-header "referer:https://web.classplusapp.com/" --add-header "x-cdn-tag:empty" -f "{ytf}" "{url}" -o "{name}.mp4"'
            elif "youtube.com" in url or "youtu.be" in url:
                cmd = f'yt-dlp {proxy_flag()} --cookies youtube_cookies.txt -f "{ytf}" "{url}" -o "{name}".mp4'
            else:
                cmd = f'yt-dlp {proxy_flag()} -f "{ytf}" "{url}" -o "{name}.mp4"'
            ...
            try:
                ...
                if "drive" in url:
                    ...
                    if "cwmediabkt99" in url:
                        ...
                        try:
                            cmd = f'yt-dlp {proxy_flag()} -o "{name}.pdf" "{url}"'
                    ...
                    try:
                        ext = url.split('.')[-1]
                        cmd = f'yt-dlp {proxy_flag()} -o "{name}.{ext}" "{url}"'
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        os.system(download_cmd)
    ...
    try:
            ...
            try:
                ...
                if "drive" in url:
                    ...
                    try:
                        ext = url.split('.')[-1]
                        cmd = f'yt-dlp {proxy_flag()} -x --audio-format {ext} -o "{name}.{ext}" "{url}"'
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        os.system(download_cmd)
